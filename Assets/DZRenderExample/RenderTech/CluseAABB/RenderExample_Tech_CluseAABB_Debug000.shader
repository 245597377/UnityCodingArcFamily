Shader "RenderExample/Tech/Cluster_Debug000"
{
	Properties
	{
	}
	SubShader
	{
		...
		Pass
		{
			...
			VertexShaderOutput main_VS(uint VertexID : SV_VertexID)
			{
				...
				vsOutput.Min = float4(0.0,0.0,0.0,1.0);
				vsOutput.Max = float4(100.0,100.0,100.0,1.0);

				float4 factor = vsOutput.Max - vsOutput.Min;
				//factor *= 0.2;
				vsOutput.Max = vsOutput.Min + factor;
